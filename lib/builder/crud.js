...
`      if (query.${attrKey}) {
        options.where['${attrKey}'] = { ${options} };
          ...
          if (attrKey === 'updated_at') {
            contentQueriesUpdate += `      selected.${attrKey} = new Date();\n`;
          ...
          if (attrKey === 'deleted_at') {
            contentQueriesRemove += `      selected.${attrKey} = new Date();\n`;
