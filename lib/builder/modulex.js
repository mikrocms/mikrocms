...
module.exports = function ModulexBuilder() {
  ...
  async function updateDatabase(connectionName) {
    ...
    for (var index in config.indexjs.database) {
      databases += `  '${index}': require('./${index}'),\n`;

      if (index === connectionName) {
        const databaseConnectionPath = path.resolve(`${config.database_path}/${index}.js`);
  ...
  '${index}': {
    ...
    'structure': require('./${index}'),
  },`;

      if (index === schemaName) {        
        const schemaStructurePath = path.resolve(`${config.schema_path}/${index}.js`);
  ...
  async function updateModel(modelName) {
    ...
    for (var index in config.indexjs.model) {
      models += `  '${index}': require('./${index}'),\n`;

      if (index === modelName) {
        const modelContentPath = path.resolve(`${config.model_path}/${index}.js`);
  ...
  async function updateLocale(languageCode) {
    ...
    for (var index in config.indexjs.locale) {
      locales += `  '${index}': require('./${index}'),\n`;

      if (index === languageCode) {
        const localeContentPath = path.resolve(`${config.locale_path}/${index}.js`);
  ...
  async function updateMiddleware(middlewareName) {
    ...
    for (var index in config.indexjs.middleware) {
      middlewares += `  '${index}': require('./${index}'),\n`;

      if (index === middlewareName) {
        const middlewareContentPath = path.resolve(`${config.middleware_path}/${index}.js`);
  ...
  async function updateService(serviceName) {
    ...
    for (var index in config.indexjs.service) {
      const service = config.indexjs.service[index];
      const endpointRouter = Array.isArray(service.router) ? service.router[1] : service.router;

      for (var endpoint in service.handler) {
        for (var httpMethod in service.handler[endpoint]) {
          const serviceNameHandler = `${endpointRouter || '/'}${endpoint}/${httpMethod}`.replaceAll('/', '_').substring(1);

          services += `  '${serviceNameHandler}': require('./${index}'),\n`;

          if (serviceNameHandler === serviceName) {
            const serviceContentPath = path.resolve(`${config.service_path}/${serviceNameHandler}.js`);
    
            console.log(`update file service, writeFile "${serviceContentPath}"`);
    
            await fs.writeFile(
              serviceContentPath,
              service.handler[endpoint][httpMethod],
              'utf8'
            );
          }
        }
  ...
  async function init() {
    ...
    const copyFiles = [
      ...
      [
        path.resolve(__dirname, '../src/module/.gitignore'),
