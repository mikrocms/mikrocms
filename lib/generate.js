 ...
 * @return  void
 */
async function service(x, httpMethod, endpoint, endpointRouter = null, moduleRouter = null) {
  ...
  try {
    ...
    if (opened.status) {
      const serviceName = `${endpointRouter || '/'}${endpoint}/${httpMethod}`.substring(1).replaceAll('/', '_');

      const defaultService =
`function ${libs.toCamelCase('service_' + serviceName)}({ env, model, locale, middleware }) {
  return [
    // all handler methods
  ];
};\n`;

      const selectedService = modulexBuilder.config.indexjs.service?.[serviceName] || defaultService;
      ...
      modulexBuilder.config.indexjs.service[serviceName] = {
      ...
      await modulexBuilder.updateService(serviceName);
      await modulexBuilder.updateIndexjs();
    } else {
      throw new Error(opened.message);
    }
