...
/**
 * Modified module crud.
 *
 * @param   string
 * @return  void
 */
async function crud(x, schemaName) {
  if (!schemaName) {
    console.log(
      '\x1b[31m%s\x1b[0m',
      'no schema name!'
  ...
  try {
    ...
    if (opened.status) {
      const crudBuilder = builder.crud(modulexBuilder);

      await crudBuilder.open();
...
`{
  ...
  "crud": {
    ...
    "delete": true
      ...
      let selectedCRUD = null;

      if (crudBuilder.config.crud !== null) {
        selectedCRUD = crudBuilder.config.crud[schemaName] || defaultCRUD;
      } else {
        selectedCRUD = defaultCRUD;
      }

      const writeCRUD = await editor.open(
        '.temp.mikrocms.crud.json',
        `{"${schemaName}": ${typeof selectedCRUD === 'object' ? JSON.stringify(selectedCRUD, null, 2) : selectedCRUD}}`,
        true,
        true
      );
      const resultCRUD = JSON.parse(writeCRUD);

      if (crudBuilder.config.crud === null) {
        crudBuilder.config.crud = {};
      }

      crudBuilder.config.crud[schemaName] = resultCRUD[schemaName];

      await crudBuilder.updateCRUD();
      await crudBuilder.init(schemaName);
    } else {
      throw new Error(opened.message);
    }
  } catch (err) {
